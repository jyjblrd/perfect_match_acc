...
c.execute("CREATE TABLE IF NOT EXISTS location_master_table \
          (location_id integer primary key, location_name text, location_no int)")

c.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_location_no ON location_master_table (location_no)")
...
if(new_name == ""):
    ...
    new_number = input("New location number: \n")

    c.execute("INSERT INTO location_master_table (location_name, location_no) VALUES(?, ?)", (new_name, new_number))
    conn.commit()
